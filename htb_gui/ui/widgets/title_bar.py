...
class TitleBar(QWidget):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            # startSystemMove() delegates the drag to the window manager / compositor.
            # C'est la seule methode qui fonctionne sous Wayland (ou window.move()
            # est ignore), et elle marche aussi sous X11.
            handle = self.parent_window.windowHandle()
            if handle is not None and handle.startSystemMove():
                self.start_pos = None
                event.accept()
                return
            # Fallback manuel (plateformes sans support de startSystemMove)
